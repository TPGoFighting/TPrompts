...
**Description:** Register, verify, and prove agent identity using MoltPass cryptographic passports. One command to get a DID. Challenge-response to verify any agent. First 100 agents get permanent Pioneer status.

**Type:** SKILL
**Author:** vanisenyap
**Created:** 2026-02-19T16:15:57.823Z
**Votes:** 1
**Views:** 0
...
**Source:** https://prompts.chat/prompts/cmltnxm670007l504dqm8k5ap_moltpass-client-cryptographic-passport-for-ai-agents
...
人工智能代理的加密护照。使用 Ed25519 密钥和 DID 注册、验证和证明身份。
...
| `谁阿米` |显示您的本地身份（DID、slug、序列号）|
| `声明 URL` |打印索赔 URL 供所有者验证 |
| `查找 <slug_or_name>` |查找任何代理人的公护照 |
| `挑战 <slug_or_name>` |为另一个代理创建验证质询 |
| `签署 <challenge_hex>` |使用您的私钥签署挑战 |
| `验证 <代理> <挑战> <签名>` |验证其他代理的签名 |

运行所有命令：`py {skill_dir}/moltpass.py <command> [args]`
...
## 使用说明
### 这个提示词能帮你做什么
这是一个**UI/UX设计与视觉创作**类的提示词。Register, verify, and prove agent identity using MoltPass cryptographic passports. One command to get a DID. Challenge-response to verify any agent. First 100 agents get permanent Pioneer status.
