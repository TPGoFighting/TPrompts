...
**Description:** Master skill for the CLAUDE.md lifecycle: create, update, and improve files using repo-verified data, with multi-module support and stack-specific rules.

**Type:** SKILL
**Author:** barisatalay
**Created:** 2026-01-28T11:15:29.152Z
**Votes:** 3
**Views:** 0
...
**Source:** https://prompts.chat/prompts/cmkxxighu0001la04sk9u5jl1_claude-md-master
...
## 何时使用
- 用户要求创建、改进、更新或标准化 CLAUDE.md 文件。 ## 核心规则
- 仅包含在存储库或配置中验证的信息。 - 切勿包含秘密、令牌、凭据或用户数据。 - 切勿包含特定任务或临时指令。 - 保持简洁：根 <= 200 行，模块 <= 120 行。 - 使用子弹；避免长篇散文。 - 命令必须可复制粘贴并源自存储库文档/脚本/CI。 - 跳过空白部分；避免填充物。 ## 强制输入（生成前分析）
- 与检测到的堆栈（根+模块）相关的构建/打包配置。 - 存储库中使用的静态分析配置（如果存在）。 - 实际模块结构和源模式（扫描真实目录/文件）。 - 要提取的每个模块的代表性源根：
  包/功能结构、关键类型和使用的注释。 ## 发现（快速+有针对性）
1. 找到现有的 CLAUDE.md 变体：`CLAUDE.md`、`.claude.md`、`.claude.local.md`。 2. 通过最少的读取来识别堆栈和入口点：
   - `README.md`，相关`docs/*`
   - 构建/打包文件（请参阅堆栈参考）
   - 运行时/配置：`Dockerfile`、`docker-compose.yml`、`.env.example`、`config/*`
   - CI：`.github/workflows/*`、`.gitlab-ci.yml`、`.circleci/*`
3. 仅提取 repo script/config/docs 中存在的命令。 4.检测多模块结构：
   - Android/Gradle：读取 `settings.gradle` 或 `settings.gradle.kts` 包含。 - iOS：检测`*.xcodeproj`/`*.xcworkspace`中的多个目标/工作区。 - 如果多个模块/目标有 `src/` 或构建配置，请规划模块 CLAUDE.md 文件。 5. 对于每个候选模块，阅读其构建文件 + 要捕获的最少文档
   模块特定的用途、入口点和命令。 6. 扫描源根：
   - 顶级包/功能文件夹和图层约定。 - 使用的关键注释/类型（每个堆栈引用）。 - 代码库中使用的命名约定。 7. 从文档或代码模式中捕获不明显的工作流程/陷阱。性能：
- 更喜欢文件列表+有针对性的读取。 - 当一个部分或符号就足够时，避免读取全文件。 - 跳过大目录：`node_modules`、`vendor`、`build`、`dist`。 ## 堆栈特定引用（模式 2）
仅当检测信号出现时才阅读相关参考：
- Android/Gradle → `references/android.md`
- iOS/Xcode/Swift → `references/ios.md`
- PHP → `references/php.md`
...
如果检测到多个堆栈，请读取多个引用。如果无法识别堆栈，则使用通用引用。 ## 多模块输出策略（检测到时强制）
- 始终创建根“CLAUDE.md”。 - 还在每个有意义的模块/目标根目录中创建“CLAUDE.md”。 - “有意义” = 有自己的构建配置和 `src/` （或同等内容）。 - 跳过仅工具目录，例如“buildSrc”、“gradle”、“scripts”、“tools”。 - 模块文件必须是特定于模块的并避免重复：
  - 包括目的、关键路径、入口点、模块测试和模块
    命令（如果有）。 - 通过`@/CLAUDE.md`引用共享信息。 ## 业务模块 CLAUDE.md 策略（所有堆栈）
...
请参阅特定于堆栈的参考，了解要检测和报告的确切模式。 ## 更新工作流程（必须遵循）
1. 仅提出有针对性的补充；显示每个文件的差异。 2. 在应用更新之前请求批准：
...
输出建议的更改并询问：
“您批准这些更新吗？（是/否）”
停止并等待用户响应，然后再继续。 **其他环境（后备）：**
如果没有可用的结构化问题工具：
1. 清楚地显示建议的更改
2. 询问：“您批准这些更新吗？回答‘是’申请，回答‘否’取消。”
3. 等待用户明确确认后再继续

3. 应用更新，保留自定义内容。如果不存在 CLAUDE.md，则提出一个新文件以供批准。 ## 内容提取规则（强制）
- 仅来自代码库：
  - 提取：使用的类型/类/注释名称，真实路径模式，
    命名约定。 - 从不：硬编码值、秘密、API 密钥、特定于业务的逻辑。 - 从不：Do/Do Not 规则中的代码片段。 ## 写入前验证
- [ ] 每个规则引用代码库中的实际类型/路径
- [ ] Do/Do Not 部分中没有代码示例
- [ ] 模式与代码库中的实际内容相匹配（未过时）

## 内容规则
- 包括：命令、架构摘要、关键路径、测试、陷阱、工作流程怪癖。 - 排除：通用最佳实践、明显信息、未经验证的陈述。 - 使用“@path/to/file”导入以避免重复。 - Do/Do Not 格式可选；仅当文件中已使用时才保留。 - 避免使用简短的复制粘贴命令之外的代码示例。 ## 现有文件策略
检测：
- 如果`<!--由claude-md-editor技能生成-->`存在→后续运行
- 否则→第一次运行
...
切勿修改`.claude.local.md`。 ## 输出
更新后，打印一份简明报告：
```

## 使用说明
### 这个提示词能帮你做什么
这是一个**编程代码生成与审查**类的提示词。Master skill for the CLAUDE.md lifecycle: create, update, and improve files using repo-verified data, with multi-module support and stack-specific rules.
